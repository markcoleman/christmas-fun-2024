...
describe('Night Before Christmas Script', () => {
  ...
  it('logs Easter Egg #1 lines if RUN_EASTER_EGG is set', async () => {
    ...
    expect(allLogs).toContain('🔎 Easter Egg #1 found: RUN_EASTER_EGG variable is set!');
    expect(allLogs).toContain('The code fairies grant you extra GitHub contributions today!');
  });
  it('logs on-call lines if ONCALL === "true"', async () => {
    ...
    expect(allLogs).toContain('Better keep one eye on your pager, just in case of an alert.');
  });
  ...
  it('does NOT log on-call lines if ONCALL is not "true"', async () => {
    ...
    expect(allLogs).not.toContain('Better keep one eye on your pager, just in case of an alert.');
  });
  it('prints the debug log if Math.random() < 0.1', async () => {
    ...
    expect(allLogs).toContain('*** Debug Log: Santa has spontaneously triggered a hotfix ***');
  });
  ...
  it('does NOT print the debug log if Math.random() >= 0.1', async () => {
    ...
    expect(allLogs).not.toContain('*** Debug Log: Santa has spontaneously triggered a hotfix ***');
  });
